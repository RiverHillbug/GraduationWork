...
namespace ProfilerDataExporter
{
		...
		public static ProfilerData GetProfilerData(int firstFrameIndex, int lastFrameIndex, string selectedPropertyPath = "")
		{
			//using (Profiler.AddSample(Profiler.SamplerType.GetProfilerData))
			{
				...
				for (int frameIndex = firstFrameIndex; frameIndex <= lastFrameIndex; ++frameIndex)
				{
					...
					{
						string currentPropertyPath = profilerProperty.propertyPath.Replace("Main Thread/", "");

						bool shouldSaveProperty = string.IsNullOrEmpty(selectedPropertyPath) || currentPropertyPath == selectedPropertyPath;
	...
	public class FunctionData
	{
		...
		public void Clear()
		{
			...
			values = new FunctionDataValue[columnNames.Length];
